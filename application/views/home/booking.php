        ...
        <div class="row">
          <div class="col-lg-6">
            ...
            <div class="form-group mb-2">
              ...
              <div class="form-group mb-2">
              <label>Pilih waktu Yang Ingin Direservasi</label>
              <small class="form-text" style="color: red;">*Wajib Dipilih</small>
              <select class="select2bs4 form-control" id="id_jam" name="id_jam" onchange="tambah_waktu(this.value)">
              </select>
            </div>
            <div class="form-group mb-2">
              <label>Pilih Meja Yang Ingin Direservasi</label>
              <small class="form-text" style="color: red;">*Wajib Dipilih</small>
              <select class="select2bs4 form-control" id="id_meja" name="id_meja" onchange="tambah_meja(this.value)">
              </select>
            </div>
            <br>
  ...
  <script>
    $(document).ready(function() {
      ...
      document.getElementById("id_jam").setAttribute("disabled", "disabled");
    ...
    function pilih_tanggal(tanggal) {
      document.getElementById("id_meja").removeAttribute("disabled");
      document.getElementById("id_jam").removeAttribute("disabled");
      ...
      $.ajax({
        type: 'GET',
        url: `<?= base_url() ?>home/getzonakosong/${tanggal}`, // Ganti URL sesuai dengan fungsi yang ada di server untuk mendapatkan waktu yang tersedia
        dataType: 'json',
        success: (hasil) => {
          let isi1 = `<option disabled selected value="">Pilih Waktu Yang Ingin Direservasi</option>`;
          hasil.forEach(function(item) {
            isi1 +=
              `<option value="${item.id_jam}|${item.waktu}">jam ${item.waktu}</option>`
          });
          $('#id_jam').html(isi1);
        }
      });
    ...
    function tambah_waktu(datazona) {
      const myArr = datazona.split("|");
      let isinya1 = `
      <b>Waktu Yang Dipilih</b> = jam ${myArr[1]}
      <input type="hidden" name="hidden_id_jam" value="${myArr[0]}">
      `;
      $('#keterangan_waktu').html(isinya1);
    }
