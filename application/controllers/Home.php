...
class Home extends CI_Controller
{
    ...
    public function getzonaKosong($tanggal)
    {
        $this->load->model('zona_model');
        $arrayzona = [];

        $getzonaReserved =  $this->db->query("SELECT * FROM booking WHERE id_jam LIKE '$tanggal%' AND status_pembayaran != 'Sudah Bayar DP'");
        foreach ($getzonaReserved->result_array() as $zona) {
            array_push($arrayzona, $zona['id_jam']);
        }
        $data['zona_not_reserved'] = $this->zona_model->get_zona_kosong_by_date($arrayzona);

        echo json_encode($data['zona_not_reserved']);
    }
